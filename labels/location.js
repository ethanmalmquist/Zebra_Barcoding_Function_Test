 ...
 * - Quantity
 *
 * Location Barcode is generated from Aisle + Bay + Level + Bin.
 */

const PRINTER_IP = "192.168.1.126";
const PRINTER_PORT = 9100;

const EXCEL_FILE = path.join(__dirname, "..", "data", "location-labels.xlsx");
...
const REQUIRED_COLUMNS = ["Aisle", "Bay", "Level", "Bin", "Quantity"];
...
function padTwoDigits(value) {
  ...
  if (!Number.isFinite(numberValue)) {
    return sanitizeZplText(trimmedValue).toUpperCase();
  }

  return String(numberValue).padStart(2, "0");
}

function buildLocationBarcode(row) {
  const aisle = sanitizeZplText(row["Aisle"]).toUpperCase();
  const bay = sanitizeZplText(row["Bay"]);
  const level = sanitizeZplText(row["Level"]);
  const bin = sanitizeZplText(row["Bin"]);

  if (!aisle || !bay || !level || !bin) {
    return "";
  }
...
function buildLabel(row) {
  ...
  const level = sanitizeZplText(row["Level"]);
  const bin = sanitizeZplText(row["Bin"]);
  const barcodeValue = buildLocationBarcode(row);
  ...
  const coords = {
    ...
    bayValueX: fieldX(SECTION_2_CENTER_X, VALUE_FIELD_W, BAY_VALUE_OFFSET_X),
    bayLabelX: fieldX(SECTION_2_CENTER_X, LABEL_FIELD_W, BAY_LABEL_OFFSET_X),
    ...
    binValueX: fieldX(SECTION_4_CENTER_X, VALUE_FIELD_W, BIN_VALUE_OFFSET_X),
    binLabelX: fieldX(SECTION_4_CENTER_X, LABEL_FIELD_W, BIN_LABEL_OFFSET_X),
  };
...
function buildPrintJob(options = {}) {
  ...
  const forceQuantityOne =
    options.forceQuantityOne ?? ["test", "preview"].includes(mode);
  ...
  if (validRows.length === 0) {
    return {
      ...
      summary: {
        ...
        rowDetails: [],
        message: "No valid completed location rows found.",
      },
    };
  }
  ...
  const zpl = rowsToPrint
    .flatMap((row) => {
      ...
      rowDetails.push({
        ...
        generatedLocationBarcode: buildLocationBarcode(row),
      });
