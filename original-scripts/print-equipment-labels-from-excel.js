...
const EXCEL_FILE = path.join(__dirname, "equipment-labels.xlsx");
...
function sendToPrinter(zplData) {
  return new Promise((resolve, reject) => {
    ...
    client.connect(PRINTER_PORT, PRINTER_IP, () => {
      ...
      console.log("Sending equipment label data...");

      client.write(zplData, () => {
        console.log("Equipment label data sent.");
        client.end();
      });
    });
...
async function main() {
  console.log("Reading equipment Excel file...");
  ...
  if (validRows.length === 0) {
    console.log("No valid completed equipment rows found.");
    return;
  }
  ...
  const zplBatch = rowsToPrint
    .flatMap((row) => {
      ...
      console.log(
        `Excel Row ${row.__excelRowNumber}: printing ${quantity} equipment label(s)`
      );
  ...
  console.log(`Total equipment labels to print now: ${totalLabels}`);

  await sendToPrinter(zplBatch);

  console.log("Equipment print job complete.");
}

main().catch((error) => {
  console.error("Equipment print failed:");
